...
export const ImageMedia: React.FC<MediaProps> = (props) => {
  ...
  let displayHeight: number | null | undefined
  let alt = altFromProps
  let src: StaticImageData | string = srcFromProps || ''
  // Payload focal point (percentages) → CSS object-position, so `fill` + object-cover
  // crops toward the part of the image the editor chose in the media library.
  ...
  if (!src && resource && typeof resource === 'object') {
    ...
    if (typeof fx === 'number' && typeof fy === 'number') objectPosition = `${fx}% ${fy}%`
        ...
        style={{
          ...
          ...(!fill
            ? {
                ...
                width: width ? `min(100%, ${width}px)` : undefined,
              }
            : {}),
        }}
